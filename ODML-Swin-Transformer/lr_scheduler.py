...
def build_scheduler(config, optimizer, n_iter_per_epoch):
    ...
    if config.TRAIN.LR_SCHEDULER.NAME == 'cosine':
        ...
        # lr_scheduler = CosineLRScheduler(
        #     optimizer,
        #     t_initial=((num_steps - warmup_steps) if config.TRAIN.LR_SCHEDULER.WARMUP_PREFIX else num_steps)/5,
        #     cycle_mul=1.,
        #     lr_min=config.TRAIN.MIN_LR,
        #     warmup_lr_init=config.TRAIN.WARMUP_LR,
        #     warmup_t=warmup_steps,
        ...
        lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer, mode='min', factor=0.1,
                                                                  patience=config.TRAIN.LR_SCHEDULER.PATIENCE, min_lr=config.TRAIN.MIN_LR, verbose=True)
